            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogContent className="max-w-md rounded-xl border border-[#D5D9DC] bg-white p-0 shadow-2xl dark:border-[#1e293b] dark:bg-[#0f172a]">
                    ...
                    <div className="flex items-center border-b border-[#D5D9DC] px-4 py-3 dark:border-[#1e293b]">
                        <Search className="mr-3 h-4 w-4 shrink-0 text-[#86888C]" />
                        <Input
                    ...
                    <div className="max-h-80 overflow-y-auto p-2">
                        <div className="px-2 py-2 text-xs font-semibold tracking-wide text-[#86888C] uppercase">
                            {query.length > 0
                                ? 'Hasil Pencarian'
                                : 'Aksi Cepat'}
                        </div>
                        <div className="space-y-0.5">
                            {filteredActions.map((action) => (
                                ...
                                >
                                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#006FCF]/[0.08] transition-colors group-hover:bg-[#006FCF]/[0.14]">
                                        <action.icon className="h-4 w-4 text-[#006FCF]" />
                                    </div>
                                    <div className="min-w-0 flex-1">
                                        <div className="truncate text-sm font-medium text-[#1A1A1A] dark:text-white">
                                            {action.title}
                                        </div>
                                        <div className="truncate text-xs text-[#86888C]">
                                            {action.description}
                                        </div>
                                    </div>
